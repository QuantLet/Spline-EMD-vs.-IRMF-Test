...
# ============================================================
# 2. 严格契合 PDF 论文定义的周期延拓（显式 3倍 Wrap）IRMF 核心分解算法
# ============================================================
def irmf_academic_decomposition(Y, h1=0.15, a=np.sqrt(2), h_min=0.008, H=0.85):
    ...
    # 💡 加上 1e-9 容错，防止 Python 浮点数精度漂移导致最后边界漏算
    while h >= (h_min - 1e-9):
        S_k = np.zeros(n)

        # 严格按照 PDF 论文 2.2 与 2.5.4 节定义进行显式周期信号延拓
...
def plot_academic_results_with_residuals_adaptive(
    ...
    # 如果极少数极端情况下层数为0，强行画出输入信号底图，确保24张图绝对不漏
    if k_total == 0:
...
if __name__ == "__main__":
    ...
    print(f"========================================================")
    print(
        f"🌐 [全维三参数网格扫描启动] 正在计算并绘制所有 {total_runs} 组超参数图像..."
    )
    print(f"========================================================")
    ...
    print("-" * 55)
    print(
        f"🏁 全空间 12 组网格扫描与 24 张高清图谱已成功完成！请前往目录查看：./{OUTPUT_DIR}/"
    )