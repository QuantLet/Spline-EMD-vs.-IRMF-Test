...
except ImportError:  # methodology-only commands do not require EMD-signal
    EMD = None
...
def run_emd_decomposition(
    ...
    """
    if EMD is None:
        raise ImportError("PyEMD is required for EMD-family experiments. Install with: pip install EMD-signal")
    emd = EMD()
    emd.nbsym = nbsym
...
def run_pyemd_family_decomposition(
    ...
    if method == "EEMD":
        ...
        model = CEEMDAN(trials=trials, epsilon=epsilon)
        model.noise_seed(random_seed)
    ...
    config = {
        ...
        "epsilon": epsilon if method == "CEEMDAN" else None,
